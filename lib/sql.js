...
function tablesToSQL(tables) {
    ...
    for (let i = 1; i < tables.length; i++) {
        ...
        if (Object.hasOwn(joinExpr, 'columns') && Array.isArray(joinExpr.columns) && joinExpr.columns.length) {
            str += ' (' + joinExpr.columns.map(identifierToSql).join(', ') + ')';
        }
        if (Object.hasOwn(joinExpr, 'on') && joinExpr.on !== null) str += ' ON ' + exprToSQL(joinExpr.on);
        if (Object.hasOwn(joinExpr, 'using')) str += ' USING (' + joinExpr.using.map(identifierToSql).join(', ') + ')';
